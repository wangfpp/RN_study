...
export default class MyDraw extends Component {
  ...
  componentDidMount() {
      this.props.navigation.addListener('willFocus',this.handleAppStateChange.bind(this));
  }
  componentWillMount() {
    this._panResponder = PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
          let len = this.MousePostions.length - 1;
          let curr = this.MousePostions[len];
          this.MousePostions.forEach(item => {
              if ((item.firstX === curr.firstX) && (item.firstY === curr.firstY)) {
                  this.MousePostions.push(item);
                  return;
              }
          });
  ...
  render() {
    ...
    return (
        <ImageBackground style={styles.container} source={require('../asset/image/back.png')} >
